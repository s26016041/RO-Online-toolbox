r"""在模組的全域裡找「跟著角色動」的候選。

⚠⚠ **2026-08-28 起，角色座標已經不是模組全域了 —— 這支工具只剩探索用途。**
   查完的結論見 GAMEDATA [MEM-047]／[MEM-048]：

   - 舊的座標全域是**小地圖的標記**，1082 張圖裡有 396 張沒有小地圖圖檔，
     那些圖上它從頭到尾不會被寫（症狀：座標 25 秒不變、還很有自信）。
   - 真正的位置分兩個來源，**都不是「跟著走路動的模組全域」**：
     進圖座標全域（只在換圖那一刻對）＋ 角色的移動元件（堆積上，用 AID 認人）。
     實作在 `services/player_position.py`，日常驗證用 `tools/verify_sigs.py`。
   - 這支工具的 `narrow` 篩出來的模組內候選**全是 A\* 節點陣列**
     （[MEM-007]，間距 0x1C、值是一串相鄰的格子）—— 那是已知的假陽性，
     不是新發現。看到它們就是「這條路走完了」。

原本的用法（下面這段還能跑，但先讀完上面再決定要不要跑）：
