...
#:
#: 2 → 3（2026-09-03 同日）：v0.5.38 還是把「角色一步都沒動」記成走不到
#: （[DAT-065]），而根因其實是**我們把目的地設在商人身上**（[DAT-066]）——
#: 站得越近越容易發生。所以 v2 檔案裡的「走不到」同樣不可信，一樣整份重學。
#: 實際被冤枉的例子：`izlude_in:57,110`（道具商人）在 14:05:11 被寫進去，
#: 而三秒後同一隻角色走去別的地方走得動。
_VERSION = 3
