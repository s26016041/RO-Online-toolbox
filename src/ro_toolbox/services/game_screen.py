...
def agree_template(dpi: int) -> QImage | None:
    """載入樣板並依 DPI 縮放。載不到回 None（呼叫端要退回比例法）。"""
    from PySide6.QtCore import Qt

    from ro_toolbox.config.paths import RESOURCES_DIR

    path = RESOURCES_DIR / AGREE_TEMPLATE_FILE
    image = QImage(str(path))
    if image.isNull():
        log.warning("載不到同意按鈕的樣板：%s", path)
        return None
    scale = (dpi or AGREE_TEMPLATE_DPI) / AGREE_TEMPLATE_DPI
...
def find_agree_button(image: QImage, dpi: int) -> tuple[int, int] | None:
    ...
    if template is None:
        return None
    screen = _shrink(_gray_array(image), _AGREE_SHRINK)
    patch = _shrink(_gray_array(template), _AGREE_SHRINK)
