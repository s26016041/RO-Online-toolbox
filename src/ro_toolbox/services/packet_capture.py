...
class PacketCapture:
    ...
    def _process_frame(self, frame: bytes, remember: bool = True) -> None:
        ...
        # ⚠ 位址與連接埠**兩個條件取聯集**，不是二選一。
        #
        # 只用位址的話，客戶端從登入伺服器換到角色伺服器（再換到地圖伺服器）
        # 的那一刻，過濾器還鎖在舊位址，看門狗要一秒才跟上 —— **角色清單就落在
        # 那個空窗裡整包不見**（實測：2026-08-25 的擷取裡完全沒有角色名）。
        # 換圖、換頻道、斷線重連都是同一個坑。
        #
        # 加上連接埠這條之後，新連線一出現就收得到，不必等看門狗換位址。
        if self._server_ip and self._server_ip in (src, dst):
            outbound = dst == self._server_ip
        elif sport in ports and dport not in WEB_PORTS:
            outbound = True
        elif dport in ports and sport not in WEB_PORTS:
            outbound = False
        else:
            # 認不出主人。可能真的是別的程式的流量，**也可能是遊戲剛開的新連線
            # 但連接埠還沒被登記到**（集合每 0.2 秒才更新，而 TCP 交握到送出
            # 帳密只隔幾毫秒 —— 登入那一包就是這樣漏掉的）。
            # 先留著，等連接埠集合更新時回頭認領（見 _replay）。
