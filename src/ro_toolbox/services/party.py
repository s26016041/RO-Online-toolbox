...
其中隊友那一筆是在第 26.8 秒他動了才來）。所以**隊友站著不動的時候我們
認不出他** —— 使用者實測回報「在白狐旁邊等 10 秒才放」，等的就是這個。
**隊友動一下就會被認出來**，之後都是即時的。

⚠ **完整的隊伍清單（`0x00FB`）抓不到**：它只在加入隊伍／進圖那一刻送，
中途開擷取就永遠等不到（實測換一次圖也沒有）。

⛔ **記憶體那條路試過，不穩**：掃「白狐的 AID ＋ 名字相鄰」找到 9 處，
其中 3 處版面一致（`AID(4) + ?(4) + 名字`），看起來很像隊伍清單 ——
但幾分鐘後再讀，那幾塊**已經被別的東西覆寫**（名字第一個 byte 變成 0，
後面接著 `izlude_in.rsw`）。那是暫時的堆積資料，不是隊伍清單。
要走這條路得先找到真正的容器，別再從「名字附近有 AID」下手。
