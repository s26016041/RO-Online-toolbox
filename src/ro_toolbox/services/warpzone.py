...
KEEP_OUT = 3
#: 傳點幾格內的怪**一律不打**。比 `KEEP_OUT` 大很多，這是故意的。
#:
#: ⚠⚠ 為什麼「不踩傳點」不夠：攻擊封包只帶 GID、不帶座標，**最後那一段路
#: 是伺服器帶的**（見 farm_bot `_ATTACK_RANGE`）—— 而且 `0x0437` 是**連續**
#: 攻擊，怪走到哪，伺服器就把角色拉到哪。於是我們的 A* 再怎麼繞開傳點都沒用：
#: 只要鎖定的怪走進傳點，角色會被**伺服器**帶著踩上去
#: （使用者實測回報「自動戰鬥會因為打怪物跑到傳送點傳出去」）。
#:
#: 所以「不打」的範圍要涵蓋「這隻怪在這一場交戰裡跑得到傳點嗎」，
#: 不只是「牠現在站在傳點上嗎」。取 `KEEP_OUT` 再往外 5 格：
#: 一拍 0.2 秒、怪一步約 0.15~0.3 秒，就算牠一路往傳點跑，
#: 每拍重驗的脫離（farm_bot `_break_off`）也來得及在牠踩到之前收手。
#:
#: 🔬 調大 = 更安全但傳點附近整片不打；調小就會退回上面那個症狀。
#: 只影響**選目標**，不影響走路（走路仍是 `KEEP_OUT`），所以放大很便宜。
NO_FIGHT = KEEP_OUT + 5
