...
class AppSettings:
    ...
    #: ⛔ 這裡以前有 `login_focus_password`（記住「登入畫面焦點在哪一格」）。
    #: **已移除，不要加回來。** 那個前提「焦點在哪一格是客戶端的固定性質」
    #: 被實機推翻了：第一個帳號登入成功之後客戶端就變成「有記住帳號」，
    #: 焦點跟著移到密碼欄，於是第二個帳號照抄上一次的答案就打錯格，
    #: 把客戶端記著的舊帳號送了出去（2026-08-31，GAMEDATA [INP-029]）。
    #: 現在每一次登入都現查，見 `auto_login._type_by_probing`。
