...
⚠ 這支工具的**離線模擬**已用合成程式碼測過（`tests/test_verify_sigs.py`），
但 **live 唯一性檢查還沒對真的客戶端跑過**（2026-08-25 遊戲維修）。
第一次跑要留意它印出來的實際數字是否合理，並把結果記進 GAMEDATA。
...
| `services/character.py`（`signatures.CHAR_STATUS`） | HP 前方固定欄位的位元組樣式 | [MEM-003] |
...
| 在哪 | 偏移 | 出處 |
|---|---|---|
| `signatures.STATUS_OFFSETS` | HP/SP `+0x00`~`+0x0C`、Base `-0x3B58`、AID `-0x3BA4`、名字 `+0x2800`、地圖 `-0x3B9C`、座標 `-0x3AD5FC`、經驗四個 int64 | [MEM-003]～[MEM-006]、[MEM-015]、[MEM-017] |
...
`assets/items.json.gz`、`mobs.json.gz`、`warps.json.gz` 是從 `RODATA/`
抽出來的（來源都是客戶端自己的 Lua 資料，不是外站抄的）。
...
| `warps.json.gz` | 傳點走錯地方 | 抽查目前地圖的傳點 |
| 地形 `.gat` | ⛔ **`available_maps()` 變 0 會讓自動打怪安靜失效** | 直接檢查它不是 0（曾因重新解包後路徑改變而歸零） |
